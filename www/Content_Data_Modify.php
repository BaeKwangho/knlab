...
if(sizeof($_POST)){
	...
	if(!isset($_POST["DC_CODE"])){
		...
		$codes = $Mem->qa("select code from nt_document_code_list where pid = ? and stat < 9",$_POST["PID"])[0];
		//등록될 코드가 저장되어 있었는지에 대해
		foreach($_POST["DC_CODE"] as $code){
			if(in_array($code,$codes)){
				continue;
			}else{
				$Mem->q("insert into nt_document_code_list (pid,code,stat,uid,dt_write) values (?,?,?,?,?)",
				array($_POST["PID"],$code,0,$Mem->user["uid"],$date));
			}
		}
		//저장된 코드가 남을건지에 대해
		foreach($codes as $code){
			if(in_array($code,$_POST["DC_CODE"])){
				...
				$Mem->q("delete from nt_document_code_list where pid = ? and code = ?",array($_POST["PID"],$code));
			}
		}
	}
...
//print_r($_POST["DC_LINK"]);
if($_POST["DC_LINK"]){
	
	$link_post="";
	for($i=0;$i<count($_POST["DC_LINK"]);$i++){
		$link_post.=$_POST["DC_LINK"][$i];
		if($i<count($_POST["DC_LINK"])-1) $link_post.=" ";
	}
}
//$Mem->insertPost("nt_document_list");
...
if($_POST["DC_COUNTRY"]){
	...
	foreach($_POST["DC_COUNTRY"] as $con){
		...
		$coq = $Mem->q("select * from nt_countrys where CTY_NM = '".$con."'")->fetch();
			if(!is_null($coq["IDX"])){
				if($stat==0){$stat=1;$Mem->q("delete from nt_document_cty_list where PID=?",$_POST["PID"]);}
				$Mem->q("insert into nt_document_cty_list (PID, CTYID, DC_DT_MODI) values (?,?,?)",array(
					$_POST["PID"], $coq["IDX"], $date));
				$countrys.=$con.",";
			}
	}
}

$Mem->q("update nt_document_list set  DC_COUNTRY=:DC_COUNTRY, DC_TITLE_OR=:DC_TITLE_OR, DC_TITLE_KR=:DC_TITLE_KR, DC_SMRY_KR=:DC_SMRY_KR, DC_DT_COLLECT=:DC_DT_COLLECT, DC_DT_WRITE=:DC_DT_WRITE, DC_URL_LOC=:DC_URL_LOC, DC_URL_EXP=:DC_URL_EXP, DC_AGENCY=:DC_AGENCY, DC_PAGE=:DC_PAGE, DC_TYPE=:DC_TYPE, DC_KEYWORD=:DC_KEYWORD, DC_CONTENT=:DC_CONTENT, DC_MEMO1=:DC_MEMO1,DC_MEMO2=:DC_MEMO2 ,DC_LINK=:DC_LINK where IDX=:IDX  ",
 array(":IDX"=>$_POST["PID"],"DC_COUNTRY"=>$countrys,":DC_TITLE_OR"=>$_POST["DC_TITLE_OR"],":DC_TITLE_KR"=>$_POST["DC_TITLE_KR"],":DC_SMRY_KR"=>$_POST["DC_SMRY_KR"],":DC_DT_COLLECT"=>$_POST["DC_DT_COLLECT"],":DC_DT_WRITE"=>$_POST["DC_DT_WRITE"],":DC_URL_LOC"=>$_POST["DC_URL_LOC"],":DC_URL_EXP"=>$_POST["DC_URL_EXP"],":DC_AGENCY"=>$_POST["DC_AGENCY"],":DC_PAGE"=>$_POST["DC_PAGE"],":DC_TYPE"=>$_POST["DC_TYPE"],":DC_KEYWORD"=>$_POST["DC_KEYWORD"],":DC_CONTENT"=>$_POST["DC_CONTENT"],":DC_MEMO1"=>$docArray,":DC_MEMO2"=>$_SESSION["TMP_CORVER"][$lastnum]["name"],":DC_LINK"=>$link_post));
...
$QS=$Mem->q("Select * from nt_categorys_join_list where PID=? ",$_GET["PID"]);
while($rs=$QS->fetch()){
	$_SESSION["SET_CATEGORY"][$rs["TYPE"]][$rs["CID"]]=1;
}
...
<div>
	...
	<div>
		<?
		$codes = $Mem->q("select code from nt_document_code_list where pid = ? and stat<9",$r["IDX"]);
		while($code = $codes->fetch()){
		?>
			<input type="hidden" id="PCODE[]" name="PCODE[]" value="<?=$code["code"]?>">
		<?	
		}
		$org_countrys = $Mem->q("select b.CTY_NM from nt_document_cty_list a join nt_countrys b on b.idx = a.CTYID where a.pid='".$r["IDX"]."';");
		while($org_country = $org_countrys->fetch()){
		?>
			<input type="hidden" id="PCOUNTRY[]" name="PCOUNTRY[]" value="<?=$org_country["CTY_NM"]?>">
		<?	
		}
		?>
		<form action="<?=SELF?>" method="post" onsubmit=" CKEDITOR.instances.contents.updateElement(); return true;"" >
		<input type="hidden" name="PID" value="<?=$r["IDX"]?>">
		<table cellpadding="0" cellspacing="0" border="0" style="width:100%;" class="table_view" >
			...
			<tr>
				...
				<th>문서안내URL</th>
				<td><input type="text" class="input_cell"  name="DC_URL_EXP" id="DC_URL_EXP"  value="<?=$r["DC_URL_EXP"]?>"  ></td>
			</tr>
			...
			<tr>
				<th>기관명</th>
				<td><input type="text" class="input_cell"  name="DC_AGENCY" id="DC_AGENCY" value="<?=$r["DC_AGENCY"]?>"   ></td> 
				<th>검색키워드</th>
				<td><input type="text" class="input_cell"  name="DC_KEYWORD" id="DC_KEYWORD"  value="<?=$r["DC_KEYWORD"]?>"  ></td> 
			</tr>
