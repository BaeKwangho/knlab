...
if($_GET["SET_TYPE"]=="true"){
	$Mem->q("insert into nt_categorys_auth_list(UID,CID,TYPE) values(?,?,?) ",array($_SESSION["SET_UID"],$_GET["SET_ID"],$_SESSION["CTYPE"]));
}else{

	$Mem->q("delete from nt_categorys_auth_list where UID= ? and CID=? and TYPE=? ",array($_SESSION["SET_UID"],$_GET["SET_ID"],$_SESSION["CTYPE"]));
}
...
<input type="checkbox" style="" name="" onclick="setup('<?=SELF?>?SET_ID=<?=$r["IDX"]?>&SET_TYPE='+this.checked);"   <?=$Mem->qs("Select count(*) from nt_categorys_auth_list where UID=? and CID=? and TYPE=? ",array($_SESSION["SET_UID"],$r["IDX"],$_SESSION["CTYPE"]))?"checked":""?>  >
				<?=$r["CT_NM"]?></td>
