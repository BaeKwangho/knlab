...
<div style="height: 50px;background-color:#2f5597;">
  <table class="menu_top">
    <td style="min-width:199px; font-size:20px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&search_reset=1')"><?if($_SESSION["AUTH"]["MID"]){$Q=$Mem->q("select CT_NM from nt_categorys where CODE like ?",$_SESSION["AUTH"]["MID"]);$r=$Q->fetch();}?><?=$r["CT_NM"]? $r["CT_NM"]:"미지정"?></td>
...
<?}else{?>
  ...
  <table class="menu_top">
    <td style="min-width:199px; font-size:20px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&search_reset=1')"><?if($_SESSION["AUTH"]["MID"]){$Q=$Mem->q("select CT_NM from nt_categorys where CODE like ?",$_SESSION["AUTH"]["MID"]);$r=$Q->fetch();}?><?=$r["CT_NM"]? $r["CT_NM"]:"미지정"?></td>
    <td class="<?=$_GET["reset_type"]=="1"?"active":""?>" style="width:20%;min-width:100px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&reset_type=1')">최근자료</td>
	<td class="<?=$_GET["TYPE"]=="전략"?"active":""?>" style="width:20%;min-width:100px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&TYPE=트렌드뉴스')">트렌드뉴스</td>
    <td class="<?=$_GET["TYPE"]=="정책동향"?"active":""?>" style="width:20%;min-width:100px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&TYPE=발간물')">발간물</td>
    <td class="<?=$_GET["TYPE"]=="산업동향"?"active":""?>" style="width:20%;min-width:100px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&TYPE=레퍼런스')">레퍼런스</td>
    <td class="<?=$_GET["TYPE"]=="보고서"?"active":""?>" style="width:20%;min-width:100px;" onclick="go('Content_view.php?<?=!$_GET["CID"]?"CID=".$_GET["CID"]:""?>&TYPE=아카이브')">아카이브</td>
  </table>
</div>
